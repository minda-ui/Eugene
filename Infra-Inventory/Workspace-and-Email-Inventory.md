...
| Fishbone Holdings Ltd | **Google Workspace, secondary domain under Construction** (owner-reported 2026-09-15) | **Inside Construction's subscription** | Migrated off 1&1's mail hosting (DNS itself is still managed in the 1&1/IONOS panel — normal for a secondary domain, no registrar move needed). Domain: **`fishboneholdings.co.uk`**. **MX verified 2026-09-15** (Minda, via the 1&1 DNS panel — Eugene's own DNS tooling stayed blocked all session, no `dig`/`nslookup`, DoH fallback rejected by the proxy): MX points to `smtp.google.com`, confirming mail routes to Google. **Open follow-up, not urgent:** the domain's SPF TXT record still references IONOS's mail servers, not Google's — an outbound-deliverability risk once this domain sends mail via Google Workspace; fix by adding `include:_spf.google.com` to the SPF record in the 1&1 panel. DNS control: **Minda** (registrar access confirmed 2026-09-15). |
...
- Holdings' SPF TXT record — still references IONOS, not Google; low-urgency fix (`include:_spf.google.com`)
  for outbound-mail deliverability once the domain actively sends via Google Workspace.
